...
def load_delay_data() -> pd.DataFrame:
    """Load train_delay_data CSV. Prefers rich data if available."""
    rich_path = DATA_DIR / "train_delay_data_rich.csv"
    path = rich_path if rich_path.exists() else DELAY_DATA
    df = pd.read_csv(path)
