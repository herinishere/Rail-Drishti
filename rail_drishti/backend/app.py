...
def cascading_delays(train_no:str=Query(...)):
    _req()
    matches=MASTER_DF[MASTER_DF["train_no"].astype(str)==str(train_no)]
    if matches.empty:
        return [{"corridor":f"Train {train_no} not found","base_delay_min":0,"cascade_delay_min":0,"cascade_risk":"Low"}]
    row=matches.iloc[0]
    name=str(row.get("train_name",f"Train {train_no}"))
    tt=_type(name)
    all_dists=pd.to_numeric(matches["distance_km"],errors="coerce").dropna()
    dist=float(all_dists.max()) if not all_dists.empty and all_dists.max()>10 else 300
    w=_weather(); now=datetime.datetime.now(); tod=_tod(now.hour); dow=now.strftime("%A")
    base=float(DELAY_PREDICTOR.predict(distance_km=dist,weather=w,day_of_week=dow,time_of_day=tod,train_type=tt))
    stops=list(matches["station_code"].dropna().unique()[:8])
    results=[]
    cascade=base
    for i,stn in enumerate(stops):
        cascade=round(base*(1+i*0.15),1)
        results.append({"corridor":f"{train_no} via {stn}","base_delay_min":round(base,1),"cascade_delay_min":cascade,"cascade_risk":"High" if cascade>30 else("Medium" if cascade>15 else"Low")})
    return results
